#!/usr/bin/env python3
"""
batch_overfit.py — Convert all overfitting rosbags → H5 + goal + video.
...
Usage:
  python scripts/batch_overfit.py [--force] [--skip-video] [--video-only]
"""
...
# ── Constants ─────────────────────────────────────────────────────────────────
BAGS_DIR = "/scratch/kvinod/bags/overfitting_data"
OUT_DIR = "/scratch/kvinod/bags/eGo_navi_overfit_data_h5"
MEDIA_DIR = os.path.join(OUT_DIR, "media")


def discover_bags():
    """Auto-discover all bag directories in BAGS_DIR."""
    dirs = sorted(glob.glob(os.path.join(BAGS_DIR, "data_collect_*")))
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch: convert overfitting bags → H5 + goal + video")
    ...
    bag_names = discover_bags()
    print(f"Discovered {len(bag_names)} bags in {BAGS_DIR}")
    ...
    # Save report
    report_path = os.path.join(OUT_DIR, "overfit_report.json")
